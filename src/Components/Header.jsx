...
const Header = () => {
  ...
  // console.log(user)
  return (
    <header className='flex items-center justify-between w-full px-[120px] py-8 z-20 relative'>
        <div className='logo'>
          ...
          <Link to="/"><img alt='logo' width={150} src={logo} className="my-filter" /></Link>
        </div>
        {
          user && 
          <div className='flex gap-3 relative user-info'>
            <img alt="user" className='profile-pic' src={user.photoURL} height={30} width={30} />
            <div className='login-details absolute bg-black w-[200px] top-10 right-0'>
            ...
            {/* <button className='font-medium py-1 px-3 text-white bg-[#e50914] rounded-md ' onClick={handleSignOut}>Sign Out</button> */}
          </div>
        }
        {
          ["/"].includes(path) && 
          <div className='flex gap-3'>
            <select className='border py-1 appearance-none pr-5 pl-3 border-white rounded-md bg-black text-white relative before:absolute before:w-[1px] before:h-[3px] before:bg-white before:right-0 before:top-0'>
